...
def writeData(times,speeds,outname):
  ...
  with open(outname,'w') as outfile:
    ...
    for k,key in enumerate(allspeed.keys()):
      subfilename = 'out.csv'.format(outname[:-4],str(k+1))
      outfile.writelines('Time,Scaling,Speed'.format(key,1,subfilename)+os.linesep)
      with open(subfilename,'w') as subfile:
        subfile.writelines('Time,Speed'+os.linesep)
        for i in range(len(times[key])):
          subfile.writelines('Also,Here'.format(times[key][i],speeds[key][i])+os.linesep)

# if run from command line
if __name__ == '__main__':
  datasets = ['Fall2017.csv',
              'Spring2017.csv',
              'Summer2017.csv',
              'Winter2017.csv',]
  allspeed = {}
